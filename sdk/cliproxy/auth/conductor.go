...
// Candidate captures an execution attempt during request routing.
// It is used to build request trace timelines in the management UI.
type Candidate struct {
	// RequestID is the per-request ID generated by the API server middleware.
	RequestID string
	// Provider is the selected upstream provider key for this attempt.
	Provider string
	// AuthID is the stable auth identifier selected for this attempt.
	AuthID string
	// AuthFile is the basename of the auth file when available (for display only).
	AuthFile string
	// Status is one of: pending, success, failed, skipped.
	Status string
	// StatusCode is the best-effort upstream HTTP status code (0 when unknown).
	StatusCode int
	// Success indicates whether the attempt succeeded.
	Success bool
	// DurationMs is the attempt latency in milliseconds (best-effort; 0 when unknown).
	DurationMs int64
	// ErrorMessage carries an optional failure summary when Status is failed.
	ErrorMessage string
	// CandidateIndex is the provider-stage index within this request.
	CandidateIndex int
	// RetryIndex is the retry index within the current provider-stage.
	RetryIndex int
}
...
type Hook interface {
	...
	// OnCandidate fires when an execution attempt is completed (success or failure).
	OnCandidate(ctx context.Context, candidate Candidate)
}
...
// OnCandidate implements Hook.
func (NoopHook) OnCandidate(context.Context, Candidate) {}
...
func (t *candidateTrace) next(provider string) (int, int) {
	if t == nil {
		return 0, 0
	}
	...
	if provider == t.lastProvider {
		t.retryIndex++
		return t.candidateIndex, t.retryIndex
	}
	t.candidateIndex++
	t.retryIndex = 0
	t.lastProvider = provider
...
func candidateAuthFile(auth *Auth) string {
	if auth == nil {
		return ""
	}
...
func candidateStatusCode(err error) int {
	if err == nil {
		return 200
	}
...
func buildCandidateAttempt(ctx context.Context, trace *candidateTrace, provider string, auth *Auth) (Candidate, time.Time) {
	candidateIndex, retryIndex := trace.next(provider)
	authID := ""
	if auth != nil {
		authID = auth.ID
	}
	return Candidate{
		...
		Provider:       provider,
		AuthID:         authID,
		AuthFile:       candidateAuthFile(auth),
		Status:         "pending",
		StatusCode:     0,
		Success:        false,
		DurationMs:     0,
...
func (m *Manager) reportCandidate(ctx context.Context, candidate Candidate, startedAt time.Time, err error) {
	if m == nil {
		return
	}
	...
	if err != nil {
		...
		candidate.StatusCode = candidateStatusCode(err)
	...
	m.hook.OnCandidate(ctx, candidate)
}

func (m *Manager) wrapStreamResultWithCandidate(ctx context.Context, streamResult *cliproxyexecutor.StreamResult, candidate Candidate, startedAt time.Time) *cliproxyexecutor.StreamResult {
	if streamResult == nil {
		return nil
	}
	...
	go func() {
		...
		m.reportCandidate(ctx, candidate, startedAt, firstErr)
	}()
...
func (m *Manager) Execute(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	for attempt := 0; ; attempt++ {
		resp, errExec := m.executeMixedOnce(ctx, normalized, req, opts, trace, maxRetryCredentials)
		if errExec == nil {
			return resp, nil
		}
		lastErr = errExec
...
func (m *Manager) ExecuteCount(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	var lastErr error
	for attempt := 0; ; attempt++ {
		resp, errExec := m.executeCountMixedOnce(ctx, normalized, req, opts, trace, maxRetryCredentials)
		if errExec == nil {
			return resp, nil
		}
		lastErr = errExec
...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	var lastErr error
	for attempt := 0; ; attempt++ {
		result, errStream := m.executeStreamMixedOnce(ctx, normalized, req, opts, trace, maxRetryCredentials)
		if errStream == nil {
			return result, nil
		}
		lastErr = errStream
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, trace *candidateTrace, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	if len(providers) == 0 {
		return cliproxyexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	...
	for {
		...
		candidate, startedAt := buildCandidateAttempt(execCtx, trace, provider, auth)
		...
		for _, upstreamModel := range models {
			...
			if errExec != nil {
				if errCtx := execCtx.Err(); errCtx != nil {
					m.reportCandidate(execCtx, candidate, startedAt, errExec)
			...
			m.MarkResult(execCtx, result)
			m.reportCandidate(execCtx, candidate, startedAt, nil)
			return resp, nil
		}
		if authErr != nil {
			m.reportCandidate(execCtx, candidate, startedAt, authErr)
			lastErr = authErr
			continue
		}
	}
}
func (m *Manager) executeCountMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, trace *candidateTrace, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	if len(providers) == 0 {
		return cliproxyexecutor.Response{}, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := req.Model
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, trace *candidateTrace, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	if len(providers) == 0 {
		return nil, &Error{Code: "provider_not_found", Message: "no provider supplied"}
	}
	routeModel := req.Model
	...
	for {
		...
		if errStream != nil {
			m.reportCandidate(execCtx, candidate, startedAt, errStream)
			if errCtx := execCtx.Err(); errCtx != nil {
				return nil, errCtx
			}
			if isRequestInvalidError(errStream) {
		...
		return m.wrapStreamResultWithCandidate(execCtx, streamResult, candidate, startedAt), nil
	}
}
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	for _, candidate := range m.auths {
		...
		if modelKey != "" && registryRef != nil {
			if models := registryRef.GetModelsForClient(candidate.ID); len(models) > 0 && !registryRef.ClientSupportsModel(candidate.ID, modelKey) {
				continue
			}
		}
