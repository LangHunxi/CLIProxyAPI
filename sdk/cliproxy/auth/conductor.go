...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, trace *candidateTrace, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		requestID := logging.GetRequestID(execCtx)
		...
		m.hook.OnCandidate(execCtx, Candidate{
			RequestID:      requestID,
			Provider:       provider,
			AuthID:         auth.ID,
			AuthFile:       authFile,
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, trace *candidateTrace, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		go func(streamCtx context.Context, streamAuth *Auth, streamProvider string, streamChunks <-chan cliproxyexecutor.StreamChunk, traceCandidate Candidate, traceStartedAt time.Time) {
			defer close(out)
			var failed bool
			var firstErr error
			forward := true
			...
			traceCandidate.DurationMs = time.Since(traceStartedAt).Milliseconds()
			if failed {
				traceCandidate.Status = "failed"
				traceCandidate.Success = false
				traceCandidate.StatusCode = 0
				traceCandidate.ErrorMessage = ""
				if firstErr != nil {
					traceCandidate.ErrorMessage = firstErr.Error()
					var se cliproxyexecutor.StatusError
					if errors.As(firstErr, &se) && se != nil {
						traceCandidate.StatusCode = se.StatusCode()
					}
				}
			...
			traceCandidate.Status = "success"
			traceCandidate.Success = true
