...
func TestCheckSystemInstructionsWithMode_StringSystemPreserved(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, "You are a helpful assistant.") || !strings.Contains(got, "<system-reminder>") {
		t.Fatalf("expected first user message to contain forwarded system reminder, got %q", got)
	}
}
...
func TestCheckSystemInstructionsWithMode_StringSystemStrict(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); got != "hi" {
		t.Fatalf("strict mode should leave the first user message unchanged, got %q", got)
	}
}

// Test case 3: Empty string system prompt does not produce a forwarded reminder
func TestCheckSystemInstructionsWithMode_EmptyStringSystemIgnored(t *testing.T) {
	...
	blocks := gjson.GetBytes(out, "system").Array()
	if len(blocks) != 3 {
		t.Fatalf("empty string system should still produce the 3 injected blocks, got %d", len(blocks))
	}
	if got := gjson.GetBytes(out, "messages.0.content").String(); got != "hi" {
		t.Fatalf("empty string system should leave the first user message unchanged, got %q", got)
	}
}

// Test case 4: Array system prompt is forwarded via the first user message reminder
func TestCheckSystemInstructionsWithMode_ArraySystemStillWorks(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, "Be concise.") || !strings.Contains(got, "<system-reminder>") {
		t.Fatalf("expected first user message to include forwarded array system prompt, got %q", got)
	}
}
...
func TestCheckSystemInstructionsWithMode_StringWithSpecialChars(t *testing.T) {
	...
	if got := gjson.GetBytes(out, "messages.0.content").String(); !strings.Contains(got, `Use <xml> tags & "quotes" in output.`) {
		t.Fatalf("forwarded system reminder text mangled, got %q", got)
	}
}
